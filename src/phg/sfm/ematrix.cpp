...
void ensureSpectralProperty(matrix3d &Ecv) {
    ...
    double singularValue = svd.singularValues().array().sum() / 2.0;
    Eigen::MatrixXd D = (singularValue * Eigen::Vector3d(1, 1, 0)).matrix().asDiagonal();
...
bool depthTest(const vector2d &m0, const vector2d &m1, const phg::Calibration &calib0, const phg::Calibration &calib1, const matrix34d &P0, const matrix34d &P1) {
    ...
    vector3d p0 = calib0.K().inv() * vector3d(m0[0], m0[1], 1.0);
    vector3d p1 = calib1.K().inv() * vector3d(m1[0], m1[1], 1.0);
    ...
    double z0 = (P0.row(2) * X)[0];
    double z1 = (P1.row(2) * X)[0];
    return z0 > 0 && z1 > 0;
}
}  // namespace
